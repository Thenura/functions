...
export function outputUpdateURLsResult(updated, urls, app) {
...
function customerAccountsUIMessage(url, extension) {
    const publicURL = `${url}/extensions/${extension.devUUID}`;
    return output.content `Preview link: ${publicURL}`;
}
...
function partnersURL(organizationId, appId) {
    return output.content `${output.token.link(`Partners Dashboard`, `https://partners.shopify.com/${organizationId}/apps/${appId}/edit`)}`.value;
}
